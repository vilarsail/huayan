#!/usr/bin/env python3
"""
apply_fix.py - 合并各批修复判断，对 docs/{N}.md 应用 ACCEPT 的修复，
生成 docs/{N}.fix.md 修复记录。

读取：
- docs/fix_{N}_pairs.json（全部问题 + 原文 + 当前译文，由 prepare_fix.py 生成）
- docs/fix_{N}_batch*.verdict.json（子 Agent 判断结果：verdict / reason / new_trans）

输出：
- 更新 docs/{N}.md（用 new_trans 替换 ACCEPT 的译文段；MISSING 情况在原文段后插入）
- 生成 docs/{N}.fix.md（格式化修复记录）

段落提取/定位规则与 check-huayan、prepare_fix.py 完全一致，保证段落号严格对应。
只替换译文块——原文段（含圈号标注 ①②）、标题、空行不动。
替换时保留原译文首行的 　　 前缀（如有）与混入译文块的页脚杂行。
从后往前处理以避免行索引偏移。
"""
...
def validate_new_trans(new_trans: str, orig: str) -> tuple[bool, str]:
    ...
    注：不校验格式 A/B 与原 trans 的一致性——check-huayan 视两种格式均为合法。
    """
...
def find_trans_block_range(lines: list[str], orig_end_idx: int) -> tuple[int, int, list[int]] | None:
    """从原文段结束行后，定位译文块的起止行索引（含）。

    返回 (trans_start, trans_end, noise_idxs)，noise_idxs 为块内杂行索引。
    若译文块不存在（MISSING），返回 None。"""
    n = len(lines)
    i = orig_end_idx + 1
    while i < n and (lines[i].strip() == '' or is_noise(lines[i])):
        i += 1
    if i >= n or not is_trans_line(lines[i]):
        return None
    start = i
    noise_idxs = []
    i += 1
    last_content_idx = start  # 最近一个非杂行、已计入块的行
    while i < n:
        line = lines[i]
        if is_noise(line):
            noise_idxs.append(i)
            i += 1
        elif is_trans_line(line):
            last_content_idx = i
            i += 1
            ...
            if j < n and is_trans_line(lines[j]):
                ...
                break
        elif i > start and not lines[last_content_idx].rstrip().endswith('*'):
            # 译文块未闭合（最近的译文行未以 * 结尾）→ 硬换行续行
            last_content_idx = i
            i += 1
        else:
            break
    return start, i - 1, noise_idxs


def locate_paragraph_block(lines: list[str], para_num: int):
    """定位第 para_num 个原文段 + 译文块的行索引范围。

    返回 (orig_start, orig_end, trans_start, trans_end, noise_idxs)，行号 0-based。
    ...
    while i < n:
        ...
        if idx == para_num:
            ...
            return orig_start, orig_end, rng[0], rng[1], rng[2]
    return None


def replace_trans_block(lines: list[str], trans_start: int, trans_end: int,
                        noise_idxs: list[int], new_trans: str) -> list[str]:
    """用 new_trans 替换 [trans_start, trans_end] 的译文块。

    保留：① 原译文首行的 　　 前缀（如有）；② 块内杂行（页脚等，追加到块尾）。"""
    old_first = lines[trans_start]
    prefix = '　　' if old_first.startswith('　　') else ''
    new_lines = new_trans.split('\n')
    new_lines[0] = prefix + new_lines[0]
    kept_noise = [lines[k] for k in noise_idxs if trans_start <= k <= trans_end]
    return lines[:trans_start] + new_lines + kept_noise + lines[trans_end + 1:]


def insert_trans_block(lines: list[str], orig_end: int, new_trans: str) -> list[str]:
    """在原文段后插入新译文块（MISSING 情况）。

    华严 docs 原文与译文多数紧邻（无空行），故直接紧随原文段插入。"""
    new_lines = new_trans.split('\n')
    return lines[:orig_end + 1] + new_lines + lines[orig_end + 1:]
...
def main():
    ...
    pairs_path = f'docs/fix_{n}_pairs.json'
    ...
    if not pairs:
        out_path = f'docs/{n}.fix.md'
        with open(out_path, 'w', encoding='utf-8') as f:
            f.write(f'# docs/{n}.md 译文修复报告\n\n')
    ...
    verdicts = {}
    batch_files_failed = []
    for path in sorted(glob.glob(f'docs/fix_{n}_batch*.verdict.json')):
    ...
    for k in sorted(to_apply, key=sort_key, reverse=True):
        ...
        if result is None:
            ...
            continue
        _, orig_end, trans_start, trans_end, noise_idxs = result
        new_trans = verdicts[k].get('new_trans', '').strip()
        if trans_start == -1:
            # MISSING 情况：原文段后无译文，插入新译文
            lines = insert_trans_block(lines, orig_end, new_trans)
        else:
            lines = replace_trans_block(lines, trans_start, trans_end, noise_idxs, new_trans)
