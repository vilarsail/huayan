#!/usr/bin/env python3
"""
verify_fix.py - 校验所有 docs/{N}.md 的修复是否已正确应用。

校验项（纯文本匹配，不调用 LLM）：
1. 每个 N 都有对应的 .check.md 和 .fix.md
2. .fix.md 头部「成立并修复」数 与 正文「✅ 已修复」段落数一致
3. .fix.md 中每个「✅ 已修复」段落的「新译文」内容必须出现在 docs/{N}.md 中
4. 可选：用 git diff 校验 docs/{N}.md 在 fix 提交后确有改动（--git 模式）

用法：
    python verify_fix.py              # 校验 1-80
    python verify_fix.py 3 5          # 校验指定文件
...
def check_git_modified(n: int) -> tuple[bool, str]:
    """用 git 判断 docs/{n}.md 在最近一次 fix 提交中是否被改动。
    ...
    try:
        ...
        # 检查该提交是否改动 docs/{n}.md
        r = subprocess.run(
            ['git', 'show', '--stat', '--pretty=format:', fix_commit, f'docs/{n}.md'],
            capture_output=True, text=True, check=True,
        )
...
def main():
    ...
    files = [int(x) for x in args] if args else list(range(1, 81))
    files.sort()
