#!/usr/bin/env python3
"""
prepare_fix.py - 修复前置：从 docs/{N}.check.md 解析问题列表，从 docs/{N}.md
提取「原文段 -> 译文段」对，合并为 docs/fix_{N}_pairs.json，并按批切块生成
docs/fix_{N}_batch*.json，供子 Agent 逐条判断「修改意见是否成立」。

仅做机械工作（解析 + 提取 + 切块），不做任何质量判断。
判断与改写全部由 LLM 子 Agent 完成。

段落提取规则与 check-huayan/prepare_check.py 完全一致，保证段落号严格对应。
"""
...
def extract_pairs(filepath: str) -> dict[str, dict]:
    """从 docs/{N}.md 提取 原文段->译文段 对（与 check-huayan 完全相同的逻辑）。"""
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    n = int(sys.argv[1])
    check_path = f'docs/{n}.check.md'
    md_path = f'docs/{n}.md'

    if not os.path.exists(check_path):
        print(f"Error: {check_path} not found. Run check-huayan first.", file=sys.stderr)
        sys.exit(1)
    ...
    if not issues:
        ...
        with open(f'docs/fix_{n}_pairs.json', 'w', encoding='utf-8') as f:
            json.dump({}, f, ensure_ascii=False, indent=2)
        print(f"No issues in {check_path}. Nothing to fix.")
        return

    # 合并 pairs + issues -> docs/fix_{N}_pairs.json
    merged = {}
    ...
    for b, batch in enumerate(batches, 1):
        with open(f'docs/fix_{n}_batch{b}.json', 'w', encoding='utf-8') as f:
            json.dump(batch, f, ensure_ascii=False, indent=2)

    print(f"Parsed {len(issues)} issues from {check_path}")
    print(f"Extracted {len(pairs)} paragraph pairs from {md_path}")
    print(f"Generated {len(batches)} batches -> docs/fix_{n}_batch*.json")
    for b, batch in enumerate(batches, 1):
