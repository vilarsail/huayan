#!/usr/bin/env python3
"""
build_report.py — 合并各批校对 issues，生成 docs/{N}.check.md 质量报告。

读取 docs/pairs_{N}.json（段对）与 docs/issues_{N}_batch*.json（各批问题），
按段落号排序输出问题明细与分类汇总。
"""
...
def main():
    ...
    if not os.path.exists(f'docs/pairs_{n}.json'):
        print(f"Error: docs/pairs_{n}.json not found. Run prepare_check.py first.", file=sys.stderr)
        sys.exit(1)

    pairs = json.load(open(f'docs/pairs_{n}.json', encoding='utf-8'))

    # 合并各批 issues（扫描所有存在的批次文件），带 JSON 校验+修复，过滤 _ 前缀键
    issues = {}
    for path in sorted(glob.glob(f'docs/issues_{n}_batch*.json')):
    ...
    lines = []
    lines.append(f'# docs/{n}.md 译文校对报告')
    lines.append('')
